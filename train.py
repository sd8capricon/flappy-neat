...
from config.game_state import FPS, GROUND_SCROLL, SCROLL_SPEED, LAST_PIPE, PIPE_FREQ, SCORE
from assets.assets import BASE_IMG, BG_IMG
from sprites.sprites import Bird, Pipe
...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        nets.append(net)
        birds.append(Bird(100, int(WIN_HEIGHT/2), True))
        birdScores.append(0)
    ...
    while running:
        ...
        for bird in birds:
            ...
            if len(pipe_group) > 0:
                ...
                bird_pipe_dist = math.sqrt(
                    (bird.rect.x-gap_mid_x)**2+(bird.rect.y-gap_mid_y)**2)
        ...
        draw_text("Alive: " + str(len(birds)), font,
                  white, 20, 100, DISPLAY_SURF)
        draw_text("Score: " + str(ITER_SCORE), font,
                  white, 290, 20, DISPLAY_SURF)
...
def train(config_file, record_stats=False, save_checkpoints=False, checkpoint_interval=10, save_winner=False):
    ...
    if save_checkpoints:
        p.add_reporter(neat.Checkpointer(
            checkpoint_interval, filename_prefix="checkpoints/neat-checkpoint-"))
    ...
    if save_winner:
        with open("winner/winner-genome.pkl", "wb") as f:
